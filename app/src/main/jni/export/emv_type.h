...
enum EMV_Status
{
	...
	EMV_SETTERMPARAMERR 			= (EMV_ERR - 27),	//!< ??????????ò??????
	EMV_PAN_NOTMATCH_TRACK2 		= (EMV_ERR - 28),	// - ????????????????
	EMV_NOT_STA_APP_SELECTED		= (EMV_ERR - 29),	// - ??????????TAG
	EMV_VALUENULL_LENERR			= (EMV_ERR - 30),	// - ????????????????
	EMV_APDU_TIMEOUT				= (EMV_ERR - 31),
    ...
    CO_SENDTAPCARDERR				= (CO_ERR -  2),	//!<二次拍卡读卡失败
    CO_RRPERR						= (CO_ERR -  3),	//!<RRP失败
    CO_CCCERR						= (CO_ERR -  4),	//!<CCC失败
    CO_PREGACBALANCEREADERR			= (CO_ERR -  5),	//!<GAC之前读余额失败
    CO_TRANSANALYSERR				= (CO_ERR -  6),	//!<交易结果分析失败
    CO_POSTGACPUTDATAERR			= (CO_ERR -  7),	//!<GAC之后PUT DATA失败
    ...
    EMV_REQ_GETCARDNO = 24,								//!<读记录完成，可以获取卡号
    EMV_REQ_READCAARD_AGAIN = 114,						//!<请求重新读卡(要求先下电后重新上电寻卡复位)
	EMV_REQ_SELECT_NEXT = 26,							//!<请求重新选择AID
	EMV_REQ_SET_BEFOREGPO = 29,							//!<请求设置beforegpo回调，所有tag设置在此回调中完成
	EMV_REQ_SET_EXCEPTIONFILE = 30,						//!<请求设置VerifyCardNo回调，在此回调中比对卡号是否在黑名单中，内核会传入卡号的ASCII值供应用比对
	EMV_REQ_SET_TAPCARD_AGAIN = 31,						//!<请求设置DispTapCardAgain回调，在此回调中设置提示用户二次拍卡，并调用相关读卡接口，读卡成功或者失败都必须给内核返回一个值，内核根据值判断是否继续流程
	EMV_REQ_SET_REVOCATIONKEY_CHECK = 32,				//!<请求设置RevocationKey回调，在此回调中比对公钥是否在回收公钥名单中
	EMV_STA_RRP_COMPLETED = 33,							//!<RRP完成
	EMV_STA_PREGACBAlANCEREAD_COMPLETED = 34,			//!<GAC之前读余额流程完成
	EMV_STA_POSTGACPUTDATA_COMPLETED = 35,				//!<GAC之后Put data流程完成

    /************************** 结束状态（流程相关） **************************/
    EMV_COMPLETED = 100,                                //!< 交易完成
    EMV_ACCEPTED_OFFLINE = 108,                         //!< 交易脱机接受
    EMV_DENIALED_OFFLINE = 109,                         //!< 交易脱机拒绝
    EMV_ACCEPTED_ONLINE = 103,                          //!< 交易联机接受
    EMV_DENIALED_ONLINE = 104,                          //!< 交易联机拒绝
    EMV_FALLBACK = 105,                                 //!< 交易降级
    EMV_SWITCH_INTERFACE = 113,							//!< 交易转界面
    EMV_SEEPHONE = 107,									//!<交易SEE PHONE
    EMV_ENDAPPLICATION = 111,							//!<交易终止
    EMV_USE_OTHERCARD = 117,							//!<交易终止，请使用其他卡片
};
...
typedef struct {
	...
	u8 extend_sel_sup;								//!<[1F30] extended selection support
    u8 cl_trans_limit_exist;                        //!< ------终端非接触交易限额存在标记
    u8 cl_trans_limit[6];                           //!< [DF20]终端非接触交易限额
    u8 cl_floor_limit_exist;                        //!< ------终端非接触脱机最低限额存在标记
    u8 cl_floor_limit[6];                           //!< [DF19]终端非接触脱机最低限额
    u8 cvm_limit_exist;                             //!< ------终端执行CVM所需限额存在标记
    u8 cvm_limit[6];                                //!< [DF21]终端执行CVM所需限额
	u8 cl_trans_limit_odcvm_exist;
	u8 cl_trans_limit_odcvm[6];                           //!< [DF22]终端非接触交易限额
    u8 term_trans_qualifiers[4];                    //!< [9F66]终端交易属性
} EMV_AIDPARAM;
