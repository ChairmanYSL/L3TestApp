...
unsigned char emvbase_avl_checkiftagempty(unsigned char *tag)
{
    EMVBASETAGCVLITEM *item;
...
unsigned char emvbase_avl_checkiftagpresent(unsigned char *tag)
{
    EMVBASETAGCVLITEM *item;
...
unsigned char emvbase_avl_checkiftagnotpresent(unsigned char *tag)
{
    EMVBASETAGCVLITEM *item;
...
unsigned char emvbase_avl_checkiftagknown(unsigned char *tag)
{
    unsigned char temptaglen = 0;
    int ret = 0;

    temptaglen = (strlen((char *)tag) > 3) ? 3 : (strlen((char *)tag));
    ret = emvbase_TagBaseLib_read_condition(tag, temptaglen, gemvbasecurtagtype, 0, 0, NULL);
    if(0 == ret)
    {
        return 1;
    }
