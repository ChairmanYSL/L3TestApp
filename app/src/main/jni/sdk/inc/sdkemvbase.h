...
typedef enum
{
    ...
    SDK_EMV_IccReturnErr =          (SDK_EMVBASE_EMV_ERR_BASE - 7),		//卡片返回数据错误,如读记录命令返回非9000等
...
typedef struct
{
    ...
    unsigned char KernelIdentifierLen;
    unsigned char KernelIdentifier[8];
	unsigned char rfuLen;
	unsigned char *rfu;
	unsigned char inputRfuLen;
}SDK_EMVBASE_CL_AIDLIST_AD;
...
typedef int (*SDK_EMVBASE_CMP_KernelID)(unsigned char transtype,unsigned char *aid,unsigned char aidlen,unsigned char asi,unsigned char* kernelid,unsigned char kernelidlen);
typedef unsigned char (*SDK_EMVBASE_SetEnableCancel)(unsigned char enable);

typedef struct{
	unsigned char BisNeedcmpKernelid;
	SDK_EMVBASE_CMP_KernelID cmpKernelID;
	SDK_EMVBASE_SetEnableCancel setEnableCancel;
}SDK_EMVBASE_ExFuncStruct;
...
extern int sdkEMVBaseSet_CMPKernelIDFunc(int(*fun_cmpKernelID)(unsigned char transtype,unsigned char *aid,unsigned char aidlen,unsigned char asi,unsigned char* kernelid,unsigned char kernelidlen));
extern unsigned char sdkEMVBaseSet_SETEnableCancel(unsigned char (*fun_setEnableCancel)(unsigned char enable));
extern s32 sdkEMVBaseSetCheckTag84(u8 flag);
